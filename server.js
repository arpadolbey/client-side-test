...
webpackServer.listen(WEBPACK_PORT, WEBPACK_HOST, () => {
  console.log(
    colors.green(
      `Starting Webpack Development Server on ${WEBPACK_HOST}:${WEBPACK_PORT}`
    )
  );
});
...
const sspServer = (() => {
  ...
  app.post('/tracking/:id', (req, res) => {
    const ssp = req.body;
    console.table({
      creativeId: ssp.creativeId,
      width: ssp.width,
      height: ssp.height,
      cpm: ssp.cpm,
      deal: ssp.deal
  ...
  app.post('*', (req, res) => {
    ...
    const size = payload.sizes
      ? payload.sizes[randomBetween(0, payload.sizes.length - 1)]
      : [1, 1];
    const ssp = req.headers.ssp;
    const cpm = Math.floor(Math.random() * 399) / 100;
    const adCode = `
            <div id="${creativeId}" style="width: ${size[0]}px; height:${
      size[1]
    }px; background:#EEEEEE">
                <script>document.querySelector("#${creativeId}").innerHTML("${creativeId} @ ${size.join(
      'x'
    )} @ ${cpm.toFixed(2)}")</script>
            </div>
        `;
    res.send(
      JSON.stringify({
...
sspServer.listen(SSP_PORT, SSP_HOST, () => {
  console.log(
    colors.green(`Starting SSP Bid Server on ${SSP_HOST}:${SSP_PORT}`)
  );
});
